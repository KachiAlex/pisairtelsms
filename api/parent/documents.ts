 ...
 * GET ?childId= — a child's uploaded documents (metadata) and issued certificates.
 *     ?id=&download=1&childId= streams the stored file bytes for a document
 *     belonging to a verified child.
 */
export default async function handler(req: ApiRequest, res: ApiResponse) {
  ...
  try {
    const { id, download, childId } = req.query
    if (!childId || typeof childId !== 'string') {
      return res.status(400).json({ error: 'childId is required' })
    }
    if (!await verifyParentChildAccess(parentId, childId, tenantId)) {
    ...
    if (download === '1' && typeof id === 'string') {
      const file = await sql`
        ...
        WHERE id = ${id} AND student_id = ${childId} AND tenant_id = ${tenantId}
        LIMIT 1
      `.catch(() => ({ rows: [] as any[] }))
