...
/**
 * /api/parent/support — the parent's own help tickets.
 * Same shape as /api/student/support.
 */
export default async function handler(req: ApiRequest, res: ApiResponse) {
  ...
  if (!parentId) return res.status(401).json({ error: 'Invalid token payload' })
  ...
  try {
    if (req.method === 'GET') {
      ...
      if (id) {
        ...
        if (!ticket || ticket.createdBy !== parentId) {
      ...
      const r = await sql`
        ...
        WHERE tenant_id = ${tenantId} AND created_by = ${parentId}
    ...
    if (req.method === 'POST') {
      ...
      if (ticketId) {
        ...
        const own = await sql`
          ...
          WHERE id = ${ticketId} AND tenant_id = ${tenantId} AND created_by = ${parentId}
          LIMIT 1
        `
        ...
        const name = (await sql`SELECT name FROM parents WHERE id = ${parentId} LIMIT 1`.catch(() => ({ rows: [] as any[] }))).rows[0]?.name || 'Parent'
        const msg = await supportTicketsApi.addComment(tenantId, ticketId, parentId, String(message).trim(), name, 'parent')
    ...
    console.error('Error in parent support:', error)
