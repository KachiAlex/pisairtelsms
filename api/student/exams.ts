...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    await sql`CREATE TABLE IF NOT EXISTS exams (
      id SERIAL PRIMARY KEY, title VARCHAR(255) NOT NULL, exam_date DATE,
      start_time TIME, end_time TIME, room VARCHAR(255),
      student_class VARCHAR(255), description TEXT, created_at TIMESTAMP DEFAULT NOW()
    )`;
