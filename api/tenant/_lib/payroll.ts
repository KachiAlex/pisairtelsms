...
// Statutory categories are regenerated by computePayroll — everything else in
// an item's deductions list is a manual entry (rules, advances, ad-hoc).
const STATUTORY_CATEGORIES = new Set(['paye_tax', 'pension_employee', 'nhf', 'nhis'])

// Shared per-staff pay computation: applies rules (two passes so %-of-gross sees
// a real gross), advance repayments, then the tax engine.
function computeStaffPay(
  basicSalary: number,
  staffRules: PayrollRule[],
  staffAdvances: SalaryAdvance[],
  taxConfig: TaxConfig | null,
  extraEarnings: EarningDeduction[] = [],
  extraDeductions: EarningDeduction[] = []
) {
  ...
  manualDeductions.push(...extraDeductions)

  const computed = computePayroll(basicSalary, earnings, manualDeductions, taxConfig)
  return { earnings, computed }
}

async function insertRunItem(
  runId: string, tenantId: string, month: string, year: number,
  staffId: string, staffName: string, basicSalary: number,
  earnings: EarningDeduction[],
  ...
  await sql`
    ...
    VALUES (${itemId}, ${runId}, ${tenantId}, ${staffId}, ${staffName}, ${basicSalary},
...
async function recomputeRunTotals(runId: string, tenantId: string): Promise<void> {
  await sql`
    UPDATE payroll_runs SET
      total_staff = (SELECT COUNT(*) FROM payroll_run_items WHERE run_id = ${runId}),
      total_gross = (SELECT COALESCE(SUM(gross_pay), 0) FROM payroll_run_items WHERE run_id = ${runId}),
      total_deductions = (SELECT COALESCE(SUM(total_deductions), 0) FROM payroll_run_items WHERE run_id = ${runId}),
      total_net = (SELECT COALESCE(SUM(net_pay), 0) FROM payroll_run_items WHERE run_id = ${runId}),
      updated_at = NOW()
    WHERE id = ${runId} AND tenant_id = ${tenantId}
  `
}

async function assertRunEditable(runId: string, tenantId: string): Promise<{ month: string; year: number }> {
  const run = await sql`SELECT status, month, year FROM payroll_runs WHERE id = ${runId} AND tenant_id = ${tenantId}`
  if (run.rows.length === 0) throw new PayrollError('Run not found')
  if (run.rows[0].status !== 'draft') {
    throw new PayrollError(`Only draft runs can be modified — this run is '${run.rows[0].status}'`)
  }
  return { month: run.rows[0].month, year: run.rows[0].year }
}

// Adds a staff member to an existing draft run — computes their pay with the
// same rules/advances/tax pipeline as run creation.
export async function addStaffToRun(runId: string, staffId: string, tenantId: string, actor?: string): Promise<PayrollRunItem> {
  const { month, year } = await assertRunEditable(runId, tenantId)

  const staffResult = await sql`
    SELECT id, name, salary FROM staff WHERE id = ${staffId} AND tenant_id = ${tenantId} AND status = 'active'
  `
  const staff = staffResult.rows[0]
  if (!staff) throw new PayrollError('Staff member not found or not active in this tenant')
  const basicSalary = Number(staff.salary) || 0
  if (basicSalary <= 0) throw new PayrollError(`${staff.name} has no salary set — update their record in Staff Directory first`)

  const dup = await sql`SELECT id FROM payroll_run_items WHERE run_id = ${runId} AND staff_id = ${staffId}`
  if (dup.rows.length > 0) throw new PayrollError(`${staff.name} is already in this run`)

  const taxConfig = await getTaxConfig(tenantId)
  const rules = (await sql`SELECT * FROM payroll_rules WHERE tenant_id = ${tenantId} AND is_active = true AND staff_id = ${staffId}`).rows.map(rowToRule)
  const advances = (await sql`SELECT * FROM salary_advances WHERE tenant_id = ${tenantId} AND staff_id = ${staffId} AND status = 'active' AND installments_paid < installments`).rows.map(rowToAdvance)

  const { earnings, computed } = computeStaffPay(basicSalary, rules, advances, taxConfig)
  await insertRunItem(runId, tenantId, month, year, staffId, staff.name, basicSalary, earnings, computed)
  await recomputeRunTotals(runId, tenantId)
  await logPayrollAudit(tenantId, runId, 'item_added', actor || 'system', { staff: staff.name })

  const item = await sql`SELECT * FROM payroll_run_items WHERE run_id = ${runId} AND staff_id = ${staffId}`
  return rowToRunItem(item.rows[0])
}

// Recomputes a draft item after an admin adjustment. Statutory deductions are
// always regenerated; manual deductions (incl. advance repayments) are kept,
// plus any extra ad-hoc lines supplied.
export async function updateRunItem(
  itemId: string,
  tenantId: string,
  updates: { basicSalary?: number; extraEarnings?: EarningDeduction[]; extraDeductions?: EarningDeduction[] },
  actor?: string
): Promise<PayrollRunItem> {
  const itemRes = await sql`SELECT * FROM payroll_run_items WHERE id = ${itemId} AND tenant_id = ${tenantId}`
  if (itemRes.rows.length === 0) throw new PayrollError('Item not found')
  const item = rowToRunItem(itemRes.rows[0])
  await assertRunEditable(item.runId, tenantId)

  const basicSalary = updates.basicSalary ?? item.basicSalary
  const taxConfig = await getTaxConfig(tenantId)
  const rules = (await sql`SELECT * FROM payroll_rules WHERE tenant_id = ${tenantId} AND is_active = true AND staff_id = ${item.staffId}`).rows.map(rowToRule)

  // Keep non-statutory deductions that already exist (advance repayments etc.)
  const keptDeductions = item.deductions.filter(d => !STATUTORY_CATEGORIES.has(d.category))
  const { earnings, computed } = computeStaffPay(
    basicSalary, rules, [], taxConfig,
    updates.extraEarnings || [],
    [...keptDeductions, ...(updates.extraDeductions || [])]
  )

  await sql`
    UPDATE payroll_run_items SET
      basic_salary = ${basicSalary},
      earnings = ${JSON.stringify(earnings)}::jsonb,
      deductions = ${JSON.stringify(computed.allDeductions)}::jsonb,
      gross_pay = ${computed.grossPay},
      total_deductions = ${computed.totalDeductions},
      net_pay = ${computed.netPay},
      paye_tax = ${computed.payeTax},
      pension_employee = ${computed.pensionEmployee},
      pension_employer = ${computed.pensionEmployer},
      nhf = ${computed.nhf},
      nhis = ${computed.nhis}
    WHERE id = ${itemId} AND tenant_id = ${tenantId}
  `
  await recomputeRunTotals(item.runId, tenantId)
  await logPayrollAudit(tenantId, item.runId, 'item_edited', actor || 'system', { staff: item.staffName, basicSalary })

  const updated = await sql`SELECT * FROM payroll_run_items WHERE id = ${itemId}`
  return rowToRunItem(updated.rows[0])
}

export async function deleteRunItem(itemId: string, tenantId: string, actor?: string): Promise<void> {
  const itemRes = await sql`SELECT run_id, staff_name FROM payroll_run_items WHERE id = ${itemId} AND tenant_id = ${tenantId}`
  if (itemRes.rows.length === 0) throw new PayrollError('Item not found')
  const runId = itemRes.rows[0].run_id
  await assertRunEditable(runId, tenantId)

  await sql`DELETE FROM payroll_run_items WHERE id = ${itemId} AND tenant_id = ${tenantId}`
  await recomputeRunTotals(runId, tenantId)
  await logPayrollAudit(tenantId, runId, 'item_removed', actor || 'system', { staff: itemRes.rows[0].staff_name })
}
