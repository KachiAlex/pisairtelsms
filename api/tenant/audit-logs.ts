  ...
  try {
    ...
    const result = await sql`
      SELECT * FROM (
        ...
        LEFT JOIN staff s ON s.id = se.user_id AND s.tenant_id = se.tenant_id
        LEFT JOIN students st ON st.id = se.user_id AND st.tenant_id = se.tenant_id
        ...
        LEFT JOIN students st2 ON st2.id = al.user_id AND st2.tenant_id = al.tenant_id::text
