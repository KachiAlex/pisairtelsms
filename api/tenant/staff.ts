...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  if (!resource || resource === 'directory') {
    if (req.method === 'GET') {
      if (id) {
        const member = await fetchStaffById(id as string, actualTenantId)
        if (!member) return res.status(404).json({ error: 'Staff member not found' })
        return res.status(200).json({ data: member })
      }
      const staff = await fetchStaff(department as string | undefined, status as string | undefined, actualTenantId)
    ...
    if (req.method === 'DELETE') {
      if (!id) return res.status(400).json({ error: 'Staff ID is required' })
      const success = await deleteStaffMember(id as string, actualTenantId)
