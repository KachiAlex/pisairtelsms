...
const attendanceRecordArbitrary = () =>
  fc.record({
    studentId: fc.uuid(),
    class: fc.constantFrom('Primary 1', 'Primary 2', 'JSS 1', 'SS 1'),
    date: fc.date({ min: new Date(2000, 0, 1), max: new Date(2100, 11, 31), noInvalidDate: true }).map(d => d.toISOString().split('T')[0]),
...
const announcementArbitrary = () =>
  fc.record({
    ...
    sentAt: fc.date({ min: new Date(2000, 0, 1), max: new Date(2100, 11, 31), noInvalidDate: true }).map(d => d.toISOString()),
  })
