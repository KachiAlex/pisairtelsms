...
export function ChildDocuments() {
  ...
  const fetchDocs = async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/parent/documents?childId=${selectedChild.id}`, {
  ...
  useEffect(() => { fetchDocs() }, [selectedChild?.id])

  const download = async (doc: Doc) => {
    if (!selectedChild) return
    const token = getAuthFromStorage()?.token
    const res = await fetch(`/api/parent/documents?id=${doc.id}&download=1&childId=${selectedChild.id}`, {
      headers: { Authorization: `Bearer ${token}` },
    })
    if (!res.ok) return
  ...
  if (!selectedChild) {
    return <p className="text-sm text-gray-500">Select a child to view their documents.</p>
  }

  return (
    <div className="space-y-8">
      <div>
        <h1 className="text-2xl font-bold text-gray-900">Documents & Certificates</h1>
        <p className="text-sm text-gray-500 mt-1">{selectedChild.name} — school documents and issued certificates</p>
      </div>
      ...
      <section>
        ...
        {loading ? (
          ...
          <p className="text-sm text-gray-500 bg-white rounded-lg border border-gray-200 p-4">
            No documents on file for {selectedChild.name}.
          </p>
        ) : (
