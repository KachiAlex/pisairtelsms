    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-gray-600 mt-1">Academic transcript and performance history</p>
        </div>
          ...
          <div className="flex-1">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mt-3 text-sm">
              ...
              <div>
                ...
                <p className="font-medium text-gray-900">{student.class} {student.arm}</p>
              </div>
              <div>
              ...
              <div>
                ...
                <p className="font-bold text-green-700">{data.cumulativeGPA.toFixed(2)}</p>
              </div>
            </div>
          </div>
        </div>
      ...
      {data.sessions.map((session, idx) => (
        <div key={idx} className="bg-white rounded-lg border border-gray-200 overflow-hidden">
          {/* Session Header */}
          <button
            onClick={() => setExpandedSession(expandedSession === idx ? -1 : idx)}
          ...
          >
            ...
            <div className="flex items-center gap-3">
              <span className={`inline-flex items-center px-2 py-1 rounded-md text-xs font-medium border ${getGradeColor(session.averageScore)}`}>
                {getGrade(session.averageScore)}
              </span>
              {expandedSession === idx ? (
            ...
            <div className="border-t border-gray-200">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 p-4 bg-gray-50">
                <div className="text-center">
                  <p className="text-2xl font-bold text-blue-700">{session.subjects.length}</p>
                  <p className="text-sm text-gray-600">Subjects</p>
                </div>
                <div className="text-center">
                  <p className="text-2xl font-bold text-green-700">{session.totalScore}</p>
                  <p className="text-sm text-gray-600">Total Score</p>
                </div>
                <div className="text-center">
                  <p className="text-2xl font-bold text-amber-700">{session.attendancePercent}%</p>
                  ...
                  <tbody className="divide-y divide-gray-100">
                    {session.subjects.map((sub) => (
                      <tr key={sub.subject} className="hover:bg-gray-50">
                        ...
                        <td className="text-center px-4 py-3">
                          <span className={`inline-flex items-center px-2 py-0.5 rounded-md text-xs font-medium border ${gradeColors[sub.grade] || 'text-gray-700 bg-gray-50 border-gray-200'}`}>
                            {sub.grade}
                          </span>
                        </td>
              ...
              <div className="p-4 bg-amber-50 border-t border-amber-100">
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-gray-600 mt-1">{session.principalComment}</p>
                  </div>
                </div>
              </div>
            ...
            return (
              <div key={idx} className="flex-1 flex flex-col items-center gap-2">
                ...
                <div className="w-full bg-gray-100 rounded-t-lg relative overflow-hidden" style={{ height: `${height * 1.2}px` }}>
                  <div
                    className={`absolute bottom-0 left-0 right-0 rounded-t-lg transition-all duration-500 ${
                      session.averageScore >= 75 ? 'bg-green-400' :
                      session.averageScore >= 60 ? 'bg-blue-400' :
                      session.averageScore >= 50 ? 'bg-amber-400' :
                      'bg-red-400'
                    }`}
                    style={{ height: '100%' }}
                  />
                ...
                <div className="text-xs text-gray-500 text-center">
                  {session.term.replace('Term', '')}<br/>{session.academicSession.split('/')[0]}
                </div>
              </div>
            )
          })}
...
function getGradeColor(score: number): string {
  ...
  return gradeColors[grade] || 'text-gray-700 bg-gray-50 border-gray-200'
}
