...
interface Notification {
  id: string
  type: 'message' | 'announcement' | 'task' | 'alert'
  title: string
  message: string
  date: string
  isRead: boolean
...
export function NotificationsDropdown() {
      ...
      if (announcementsRes?.ok) {
        ...
        notificationsList.push(
          ...announcements.slice(0, 3).map((a: any) => ({
            ...
            title: a.title,
            message: a.content?.slice(0, 100) + (a.content?.length > 100 ? '...' : '') || '',
  ...
  const markAsRead = async (notificationId: string) => {
    try {
      ...
      // Extract original ID and type
      const [type, id] = notificationId.split('-')

      if (type === 'msg') {
        await fetch(`/api/staff/messages/${id}/read`, {
          method: 'POST',
          headers: { Authorization: `Bearer ${token}` },
        })
      }
    ...
    try {
      ...
      await Promise.all(
        unreadMessages.map(n => {
          const [, id] = n.id.split('-')
          return fetch(`/api/staff/messages/${id}/read`, {
            method: 'POST',
            headers: { Authorization: `Bearer ${token}` },
          })
        })
        ...
        <div className="absolute right-0 top-full mt-2 w-80 sm:w-96 bg-white rounded-lg shadow-lg border border-gray-200 z-50 overflow-hidden">
          ...
          <div className="border-t border-gray-200 p-2 bg-gray-50">
            <button
              onClick={() => {
                setIsOpen(false)
                window.location.href = '/staff/communications'
              }}
